...
/** ARM standard SSDT Generic Device Table Generator
  ...
  )
{
      ...
      (GenericDeviceInfo->InterruptResourceToken != CM_NULL_TOKEN))
  {
    ...
    if (GenericDeviceInfo->InterruptResourceToken != CM_NULL_TOKEN) {
      Status = GetEArmObjGenericInterrupt (
                 CfgMgrProtocol,
...
{
  EFI_STATUS                  Status;
  CM_ARM_GENERIC_DEVICE_INFO  *GenericDeviceInfo;
  ...
  *Table = NULL;
  ...
  Status = GetEArmObjGenericDeviceInfo (
             CfgMgrProtocol,
             CM_NULL_TOKEN,
             &GenericDeviceInfo,
             &GenericDeviceCount
